...
import fsoft.xdev.mock.dao.imp.TrustRegionDao;
import fsoft.xdev.mock.models.TrustRegion;

public class TrustRegionDaoTest {

	@Mock
	private ITrustRegionDao mockingTrustRegionDao;
	@Before
	public void setUp() throws Exception {
		mockingTrustRegionDao = mock(TrustRegionDao.class);
		
	}

	@After
	public void tearDown() throws Exception {
		mockingTrustRegionDao = null;
	}

	@Test
	public void testAdd_ExpectionTrue() {
		TrustRegion trustRegion = new TrustRegion(2);
		mockingTrustRegionDao.add(trustRegion);
		verify(mockingTrustRegionDao).add(trustRegion);		
	}

	@Test
	public void testEdit_ExpectionTrue() {
		TrustRegion trustRegion = new TrustRegion(2);
		mockingTrustRegionDao.edit(trustRegion);
		verify(mockingTrustRegionDao).edit(trustRegion);
	}
	...
	public void testFindRange_All_Active_ExpectionNotEmptyList() {
		...
		mockingTrustRegionDao.findRange(from, to, filterKey, filterActive);
		verify(mockingTrustRegionDao).findRange(from, to, filterKey, filterActive);
	}
	...
	public void testCount_All_InActive_ExpectionNotZeroItems() {
		String filterKey = "All";
		boolean filterActive = false;
		mockingTrustRegionDao.count(filterKey, filterActive);
		verify(mockingTrustRegionDao).count(filterKey, filterActive);
	}
