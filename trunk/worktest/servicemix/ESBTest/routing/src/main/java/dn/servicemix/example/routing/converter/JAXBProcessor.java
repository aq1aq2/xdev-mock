...
public class JAXBProcessor implements InitializingBean {
	...
	String contextString = "dn.servicemix.example.xml";
	...
	/**
	 * Process a Camel exchange with a XML payload by unmarshalling it using
	 * JAXB. Note that there must be only one element in the CXF body.
	 * 
	 * @param exchange
	 * @throws JAXBException
	 */
	public synchronized void getXml(Exchange exchange) throws JAXBException {
		CxfPayload<SoapHeader> requestPayload = exchange.getIn().getBody(
				CxfPayload.class);
		List<Element> inElements = requestPayload.getBody();
		if (inElements.size() == 1) {
			Source xmlSource = new DOMSource(inElements.get(0));
			exchange.getIn().setBody(xmlSource);
		}
	}
